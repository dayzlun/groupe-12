...
const toThreeHourForcastType = (raw) => {
  return raw.map(doc => {
    const weather = doc.weather.map(({description, icon}) =>({description, icon}))
    return {
      weather,
      date: doc.dt_txt,
      temp: doc.main.temp,
...
console.log(JSON.stringify(toThreeHourForcastType(list), null, 2));
